    ...
    if (pendingFile) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          link.href = URL.createObjectURL(pendingFile);
          link.download = fileName;
          link.click();
          setFormData(prev => ({ ...prev, photo: `/photos/${fileName}` }));
          alert(`上传失败，照片已下载为 "${fileName}"\n请手动保存到 public/photos 文件夹`);
        }
      } catch {
        const timestamp = Date.now();
        const ext = pendingFile.name.split('.').pop();
      ...
      <div className="modal show">
        <div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            {member && onDelete && (
              <button className="btn-delete" onClick={onDelete}>删除此人</button>
            )}
